...
def processDimm(id, q, dimmList, endTime, startTime):
    for dimm in dimmList:
        ...
        df = df[(df['err_time'] <=  firstUER)].reset_index(drop=True)
        # 删去不在采样区间内的记录
        df = df[(df['err_time'] <  endTime) &(df['err_time'] >=  startTime)].reset_index(drop=True)
...
def genDataSet(index, startTime,endTime):
    
    fileName = os.path.join(DATA_SET_PATH, "{}.csv".format(index))
    ...
    for i in range(cpuCount):
        ...
        processList.append(Process(target=processDimm, args=(i,q, subDimm, endTime, startTime)))
        
    pMerge = Process(target=mergeFunction, args=[q, fileName])
    pMerge.start()
...
DATA_SET_PATH = 'time'
startTime = datetime(year=2019,month=7,day=1)
endTime = datetime(year=2023,month=4,day=1)

predictTime = timedelta(days=60)
observeTime = timedelta(days=60)
sliceTime = timedelta(days=60)
...
while True:
    ...
    currentEndTime = currentStartTime + observeTime
    if currentEndTime > endTime:
        break
    genDataSet(index,currentStartTime,currentEndTime)

    currentStartTime = currentStartTime + sliceTime
    index += 1
